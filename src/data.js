...
function getOptions({format, schema}) {
  if (schema.enum) {
    if (typeof schema.enum[0] === 'string') {
      return schema.enum.map(e => ({ answer: e, label: e}))
    }

    if (format === 'Currency') {
      return schema.enum.map(e => ({ answer: e, label: `$${e.toLocaleString('en', { minimumFractionDigits: 2 })}`}))
    }
  }
...
export async function createQuote({
  ...
  try {
    axios({
      ...
      const readyCategoriesKeys = categoryList.filter(key => input.categories[key].status === 'Ready')
        ...
        subSectionKeys.forEach(subSectionKey => {
          ...
          if (subSection.properties) {
            ...
            propertyKeys.forEach(subSectionPropertyKey => {
              const subSectionProperty = subSection.properties[subSectionPropertyKey]; // business

              // TODO need to account for billing section separately
              if (subSectionProperty.schema && category.name !== 'billing') {
                const input = {
                  ...
                  type: determineType(subSectionProperty.schema),
...
export async function updateQuote(quote, input, setQuoteValues, setInputValues, setInputsByCategory) {
  try {
    axios({
      headers: {Authorization: authorizationHeader},
      method: 'put',
